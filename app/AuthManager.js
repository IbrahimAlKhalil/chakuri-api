...
class AuthManager {
  ...
  async attempt() {
    ...
    const validation = await validate(request.all(), {
      ...
      username: this.isMobile(this.helpers.bnToEn(username)) ? 'required|min:11|max:14' : 'required|email',
  ...
  async checkCredentials(username, password, userTypeId) {
    const user = await User.query()
      ...
      .where(this.isMobile(this.helpers.bnToEn(username)) ? 'mobile' : 'email', this.helpers.truncateMobile(this.helpers.bnToEn(username)))
      .select('id', 'password')
      .first();
